...
<script type="text/javascript">
  ...
  function initializeProductResultInsertDialog(processInfo, productsMaterialSize, productsSpecSize, productPlans) {
    ...
    if(processInfo[0].processComponentNo > 1){
    	 ...
    	 $("#start").show();
    	 $("#finish").show();
    	 if(processInfo[0].processComponentNo == 6){
    		 $("#check").show();
    		 $("#cancelDialogProduct").css({"margin-left" : "270px"});
    	 }else{
    		 $("#check").hide();
    		 $("#cancelDialogProduct").css({"margin-left" : "380px"});
    	 }
    }else{
  ...
  function productsCheck() {
    var updateUrl = "${f:url('/product/productResultInput/productsCheck/')}";
...
<div id = "insertResultDialog"  style="display: none; background-color:#BDD7EE;">
  ...
  <div style="margin-top:15px;">
    ...
    <input type="button" id="check" value="<bean:message key="check"/>" onclick="productsCheck()" style="width: 110px; margin-left:20px;"/>
    <input type="button" id="cancelDialogProduct" value="<bean:message key="cancel"/>" onclick="closeProductsDialog()" style="width: 110px; margin-left:380px;"/>
  </div>
