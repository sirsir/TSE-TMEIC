...
public class ProductAction extends AbstractAction {
	...
	public String processOfProduct() {
		...
		try {
			...
			List<MProcessComponent> mProcessComponents = mProcessComponentService.findByPartNoOrderByProcessOrder(partNo);
	...
	private void registerValidate(List<MProductDto> products) {
		...
		for (MProductDto product : products) {
			...
			ValidateUtil.required("part.no", product.partNo);
			ValidateUtil.maxlength("part.no", product.partNo, 10);

			ValidateUtil.required("part.name", product.partName);
			ValidateUtil.maxlength("part.name", product.partName, 30);

			ValidateUtil.required("product.plating.machine", product.platingMachine);
	...
	private void registerExe(List<MProductDto> products) {
		...
		for (MProductDto product : products) {
			...
			final String partNo = product.partNo;
			...
			switch (product.crud) {
				...
				case "U":
					...
					for (MProcessComponentDto process : product.process) {
						...
						mProcessComponent.processOrder = 0;
						mProcessComponent.partNo = partNo;
						...
						for (MSpecProductComponentDto specProduct : process.specProducts) {
							...
							mSpecProductComponent.partNo = partNo;
						...
						for (MSpecProcessComponentDto specProcess : process.specProcesses) {
							...
							mSpecProcessComponent.partNo = partNo;
						...
						for (MMaterialComponentDto material : process.materials) {
							...
							mMaterialComponent.partNo = partNo;
			...
			switch (product.crud) {
				case "U":
					productPlanService.rescheduleOfPartNo(partNo);
					break;
			}
		}
	...
	public String delete() {
		...
		try {
			String partNo = request.getParameter("deletePartId");

			boolean isPlanning = productPlanService.isProductPlanning(partNo);

			if (isPlanning) {
				throw new ValidationException(MessageResourcesUtil.getMessage(locale(), "errors.product.delete.planning"));
			}

			mProductService.deleteById(partNo);
