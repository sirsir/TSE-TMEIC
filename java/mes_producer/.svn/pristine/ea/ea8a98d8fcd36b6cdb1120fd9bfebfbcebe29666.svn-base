...
function createProcessGrid() {
  ...
  processGrid.prop.onSelectRowEx = function (rowId){
    ...
    var processComponentNo = $(processGrid.gridId).getCell(rowId, "processComponentNo");
    
    $("#qualityLabel").val("");
    if(processComponentNo > 4){
    	$("#quality-Label").show();
    	if(processComponentNo == 6){
    		$("#ticket-printing").show();}
    }else {
        $("#ticket-printing").hide();
        $("#quality-Label").hide();
    }
...
function setProcessProductGrid(rowId, productPlanNo, processComponentNo) {
	if(processComponentNo == 4){
		$(processProductGrid.gridId).showCol("qualityLabel");
	}else{
		$(processProductGrid.gridId).hideCol("qualityLabel");
	}
