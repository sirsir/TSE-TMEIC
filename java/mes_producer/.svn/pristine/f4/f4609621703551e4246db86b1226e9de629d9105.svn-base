...
<tiles:insert template="/WEB-INF/view/common/layout.jsp" flush="true">
  ...
  <tiles:put name="content" type="string">
    ...
    var requiredOfArrivalNo = true;

    if (${arrivalNoNumberingMethod} == "1") {
      requiredOfArrivalNo = false;
    }
    ...
    function doSearch() {
      $("#mainForm").attr("action", "${f:url('/product/arrival/')}");
      $("#mainForm").submit();
    }
      ...
      dialogUtils.confirm(
        ...
        function() {
          ...
          if (grid.deleteRow(rowId)) {
            ...
            ajaxPostJson({
              url: "${f:url('/product/arrival/delete/')}",
              data: {
                deleteArrivalNo : $(grid.gridId).getCell(rowId, "arrivalNo")
              }
            });
          }
        }
